...
func getSvpAbshareDP1(active bool) string {
...
func getSvpYse1(url string) string {
	// 发起 HTTP GET 请求
	client := resty.New()
	resp, errs := client.R().
		SetHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36").
	...
	// 找到最后一个元素
	pre := doc.Find(`[type="application/json"]`).Last()
	if pre.Length() <= 0 {
		panic("未找到元素")
	}
	var data []json.RawMessage
	err = json.Unmarshal([]byte(pre.Text()), &data)
	if err != nil {
		panic(errors.New(fmt.Sprintf("反序列化 JSON 失败:%s", err)))
	}
	...
	for _, raw := range data {
		...
		/*var st time.Time
		if json.Unmarshal(data[i+1], &st) == nil {
			// 解析UTC时间字符串
			// 使用 RFC3339 格式来解析 ISO 8601 格式的时间
			//t, err := time.Parse(time.RFC3339, st)
			//if err == nil {
			// 转换为北京时间 (UTC+8)
			beijingLoc, err := time.LoadLocation("Asia/Shanghai")
			if err != nil {
				// 如果无法加载时区，使用 FixedZone
				beijingLoc = time.FixedZone("CST", 8*60*60)
			}
			beijingTime := st.In(beijingLoc)
			log.Println("北京时间:", beijingTime.Format(time.DateTime))
			//}
		}*/
...
func getSvpYouneed() string {
	...
	cri := client.R()
	resp, errs := cri.Get("https://github.com/freevpn2025/freevpn2025.github.io/raw/refs/heads/main/README.md")
	if errs != nil {
		panic(errors.New(fmt.Sprintf("HTTP GET 请求失败:%s", errs)))
	}
	if resp.StatusCode() != 200 {
	...
	links := extractLinksFromSection(string(resp.Body()), "免费V2ray节点订阅链接")
	// 2. 检查链接是否有效且为今天
	isUpToDate := false
	if len(links) > 0 {
		// 检查第一个链接的日期
		// 匹配 8 位数字日期
		matches := regexp.MustCompile(`(\d{4})(\d{2})(\d{2})`).FindStringSubmatch(links[0])
		if len(matches) > 0 {
			linkDate := matches[0] // 返回完整匹配 20251205
			loc, err := time.LoadLocation("Asia/Shanghai")
			var today time.Time
			if err != nil {
				// 如果环境没有 timezone 数据，简单的 hack：UTC + 8
				today = time.Now().UTC().Add(8 * time.Hour)
			} else {
				today = time.Now().In(loc)
			}
			if linkDate == today.Format("20060102") {
				isUpToDate = true
			}
		}
	}
	if !isUpToDate {
		links = youneedTodayLinks()
	}

	results := make(chan string) // 开启通道接收结果，可以是无缓冲，也可以带缓冲
	var wg sync.WaitGroup
	for _, text := range links {
		...
		go func() {
			...
			results <- string(decoded) // 发送结果
		}()
	}
	go func() {
		// 等待所有协程完成
		wg.Wait()
		close(results) // 关闭通道，通知主线程循环结束
	}()
	// 合并结果
	joiner := utils.NewStringJoiner("\n")
	// 主线程直接遍历读取，直到通道被关闭
